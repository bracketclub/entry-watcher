...
class Entry {
  constructor (options) {
    ...
    _.extend(this, _.pick(options, 'getId', 'logger', 'finder', 'save', 'locks', 'onSave', 'onError'))
    ...
    if (this.originalData) {
      this.logger.info('[ORIGINAL DATA]', JSON.stringify(this.originalData))
    }
  ...
  hasBracket (cb) {
    this.logger.info('[HAS BRACKET]', JSON.stringify(this.originalData))

    this.finder[this.type](this.originalData, (err, result) => {
      this.logger.info('[HAS BRACKET RESULT]', JSON.stringify({err, result}))
      ...
      if (err && this.originalData.truncated && this.getId) {
        ...
        this.logger.info('[GET ID FALLBACK]', JSON.stringify({ error: err, tweet: this.originalData }))

        return this.getId(this.entry.data_id, (getIdErr, getIdResult) => {
          this.logger.info('[GET ID HANDLER]', JSON.stringify({ error: getIdErr, tweet: getIdResult }))

          cb(getIdErr, getIdResult)
        })
      }

      if (err) {
        return cb(new Error(`${this.type} does not contain a bracket: ${this.entry.link}`))
      }
  ...
  save () {
    this.isValid((err, bracket) => {
      if (err) {
        ...
        this.logger.info('[SAVE]', JSON.stringify(_.pick(this.entry, 'username', 'bracket')))
